...
public class Experto {
    ...
    public List<RutaFabricacion> buscarRutaFabricacionCT(CentroTrabajo codigo) {

        List<RutaFabricacion> listaRutaFabricacion = new ArrayList<>();

        centroTrabajo = codigo;
        DTOCriterio c0 = new DTOCriterio();// criterio 0
        c0.setAtributo("codigoProductoTerminado");
        c0.setOperacion("=");
        c0.setValor(codigo);
    ...
    public void modificarSeleccionRF(List<RutaFabricacion> listaRutaFabricacion) {
        //error elimina todo y luego crea

        List<RutaFabricacion> listRutaFabricacion = iniciarRutaFabricacion(productoTerminado);
        
        System.out.println(listRutaFabricacion.size() + " hikaaaak es el tamali" + productoTerminado.getNombreProductoTerminado());
        /*for (int i = 0; i < listaRutaFabricacion.size(); i++) {
            int codigo = listaRutaFabricacion.get(i).getCodigoRutaFabricacion();
            bajaRF(codigo);
        }*/
        
        for (RutaFabricacion rf: listRutaFabricacion){
           bajaRF(rf.getCodigoRutaFabricacion());
        }

        altaRutaFabricacion(listaRutaFabricacion);
    ...
    public void modificarSeleccionMaq(Maquinaria maquinariaM) {
        ...
        maquinaria.setCodigoCentroTrabajo(centro);
...
//            List<RutaFabricacion> listRuta = buscarRutaFabricacionCT(centroTrabajo);
//            System.out.println(listRuta.size()+ "Tamañooooo");
//
//
//            if (listMaq.isEmpty() && listRuta.isEmpty()) {
//                try{
//                
//                    FachadaPersistencia.getInstance().eliminar(centroTrabajo);
//                }
//                catch(ConstraintViolationException logExceptions){
                    JOptionPane.showMessageDialog(null, "No se puede eliminar materia prima porque esta asociada a una ruta de fabrcacion o maquinaria");
//                    
//                }
//            }
//            else {
//                JOptionPane.showMessageDialog(null, "No se puede eliminar materia prima porque esta asociada a una maquinaria");
//            }
//        } catch (IndexOutOfBoundsException e) {
//    
//                    JOptionPane.showMessageDialog(null, "No se puede eliminar materia prima porque esta asociada a una tuta de fabrcacion");
//                    
//                
//        }
...
try{
    ...
    public void altaRutaFabricacion(List<RutaFabricacion> listRutaFabricacion) {
        ...
        for (int i = 0; i < listRutaFabricacion.size(); i++) {
            System.out.println(listRutaFabricacion.get(i).getCodigoCentroTrabajo().getNombreCentroTrabajo() + "dentro del for nombre");
            int codigo = listRutaFabricacion.get(i).getCodigoRutaFabricacion(); //obtengo el codigo
System.out.println("}codufog¿" + listRutaFabricacion.get(i).getCodigoRutaFabricacion());
            DTOCriterio c1 = new DTOCriterio();
            ...
            try {
                ...
                if (rutaFabricacion != null) {
                    ...
                    System.out.println("Ruta a guardar " + listRutaFabricacion.get(i).getCodigoCentroTrabajo().getNombreCentroTrabajo());
                       FachadaPersistencia.getInstance().guardar(listRutaFabricacion.get(i));
                }
  
            } catch (IndexOutOfBoundsException e) {
                    System.out.println("Ruta a guardar catch " + listRutaFabricacion.get(i).getCodigoCentroTrabajo().getNombreCentroTrabajo());
                 FachadaPersistencia.getInstance().guardar(listRutaFabricacion.get(i));
            }
